...
wait_for_central_reconciliation() {
    ...
    for i in $(seq 1 90); do
        ...
        numDeployments="$(curl -sSk --config <(escape user "admin:$ROX_ADMIN_PASSWORD") "https://$API_ENDPOINT/v1/summary/counts" | jq '.numDeployments' -r)"
...
force_rollback() {
    ...
    local upgradeStatus
    upgradeStatus=$(curl -sSk -X GET --config <(escape user "admin:$ROX_ADMIN_PASSWORD") https://"${API_ENDPOINT}"/v1/centralhealth/upgradestatus)
